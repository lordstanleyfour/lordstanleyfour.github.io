...
const lunate = new Outline(300, 0, 600, 600, "lunateoutline", "lunate", 325, 348, 349, 350);
const triquetrum = new Outline(300, 0, 600, 600, "triquetrumoutline", "triquetrum", 302, 303, 326, 327);
const pisiform = new Outline(300, 0, 600, 600, "pisiformoutline", "pisiform", 327);
const hamate = new Outline(300, 0, 600, 600, "hamateoutline", "hamate", 277, 278, 279, 301, 302, 325);
...
function highlightHandler (){
    outlineArray.forEach(outline => {
        ...
        gameGrid.forEach(cell => {
            //go through each highlight cell and check collision with mouse
            if (cell.number === a || cell.number === b || cell.number === c || cell.number === d || cell.number === e || cell.number === f || cell.number === g || cell.number === h || cell.number === i || cell.number === j) {
                if (mouse.x && mouse.y && collision(cell, mouse)) {
                    outlineArray.forEach(outline => {
                        outline.draw();
                    })
                }
...
//basic functions
////
//find a PA wrist and make it the background
//draw outlines for the bones and make them transparent
//add them as img elements then add to the outline array
